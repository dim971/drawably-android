...
internal class EngineGoldenTest {
    ...
    fun `shape primitives match`() {
        val mismatches = goldens.shapes.mapNotNull { case ->
    ...
    fun `boil frames match`() {
        val mismatches = goldens.variants.flatMap { case ->
            val produced = Rough.variants(
    ...
    fun `every control layer matches`() {
        val mismatches = goldens.controls.flatMap { case ->
            val produced = Rough.variants(
    ...
    fun `arrow layer matches`() {
        val mismatches = goldens.arrows.flatMap { case ->
            val a = case.args
            val produced = Rough.variants(
    ...
    fun `the fixtures cover the whole surface`() {
        ...
        assertEquals(26, goldens.controls.map { it.layer }.toSet().size)
    }
}
